...
public class EvTboxService
{
    ...
    {
        int temp = 0;
        ArrayList<EvTbox> eyTboxlist = new ArrayList<>();
        for (EvTbox evTbox : eyTboxs)
        {
            temp++;
            if (temp % 500 == 0)// 每500条新增一次
            {
                evTboxMapper.addEvTboxsByExcel(eyTboxlist);
                eyTboxlist = new ArrayList<>();
            }
            eyTboxlist.add(evTbox);
        }
        evTboxMapper.addEvTboxsByExcel(eyTboxlist);
    }
}
